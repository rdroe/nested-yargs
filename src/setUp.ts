...
type LookerUpperCaller = (modules: Modules, input: string[], commands: (number | string)[]) => Promise<Result>
...
const makeLookUpAndCall = async (yargs: any): Promise<LookerUpperCaller> => {
    ...
    async function lookUpCallFn(modules: Modules, input: string[], commands: (number | string)[]): Promise<Result> {
        // Universal options
        let yargsOptions = {
            'commands': {
                array: true,
